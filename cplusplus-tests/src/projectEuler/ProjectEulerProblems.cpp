...
long long problem_011_greatestProductAdjacentInGrid(string input, int gridWidth, int gridHeight, int sequenceLength)
{
	IntGrid intGrid = IntGrid(input, gridWidth, gridHeight);
	long long product = 0;
	long long greatestProduct = 0;

	for (size_t row = 0; row < intGrid.size(); row++)
	{
		for (size_t col = 0; col < intGrid.size(); col++)
		{
			product = intGrid.greatestProductFromCoord(row, col, sequenceLength);
